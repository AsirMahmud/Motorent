...
import { User as UserIcon, CheckCircle2, ShieldCheck, FileCheck2, Bike } from 'lucide-react';

type DocKey = 'nid' | 'license';
...
export default function RenterSignupPage() {
  ...
  const allDocsReady = docs.nid.url && docs.license.url;
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!allDocsReady) {
      setError('Please upload both NID and driving license.');
      return;
    }
    setSubmitting(true);
    try {
      const response = await fetch('/api/auth/signup/renter', {
        method: 'POST',
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-primary/10 flex flex-col font-sans">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <Card className="w-full max-w-md p-10 text-center rounded-3xl shadow-2xl border-none">
            ...
            <h2 className="text-3xl font-black mb-2">Application Submitted!</h2>
            <p className="text-muted-foreground mb-4 text-lg">Your KYC documents are under review.</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-amber-700 text-xs mt-1">
                    Admin will review your NID and driving license. You will receive an email
                    within 24 hours once approved or if any documents need resubmission.
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-xl">
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-primary rounded-2xl shadow-xl shadow-primary/30 mb-3">
              <Bike className="text-white" size={26} />
            </div>
            <h1 className="text-3xl font-black uppercase italic tracking-tighter">Renter sign up</h1>
            <p className="text-muted-foreground mt-1 text-sm">
              Submit your NID and driving license. Admin will verify within 24 hours.
            </p>
          </div>
          ...
          <Card className="p-6 sm:p-8 rounded-3xl shadow-2xl border-none">
            <form className="space-y-8" onSubmit={handleSignup}>
              ...
              <div className="space-y-4 border-t border-border pt-6">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">Both documents are required. JPG, PNG, or PDF.</p>
                  </div>
                  {allDocsReady ? (
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-800">
                      <FileCheck2 className="h-3.5 w-3.5" /> All set
                    </span>
                  ) : null}
                </div>
              ...
              {error ? <p className="text-red-600 text-sm font-medium bg-red-50 p-3 rounded-xl">{error}</p> : null}
              ...
              >
                {submitting ? 'Submitting…' : uploadBusy ? 'Uploading…' : 'Submit KYC & Sign Up'}
              </Button>

              <p className="text-center text-sm text-muted-foreground">
                Already have an account?{' '}
                <Link href="/login/renter" className="text-primary font-bold">
                  Sign in
                </Link>
                {' · '}
