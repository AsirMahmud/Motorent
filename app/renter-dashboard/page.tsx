...
import {
  ...
  ShieldCheck, AlertCircle, TrendingUp, TrendingDown, History,
...
export default function RenterDashboardPage() {
  ...
  const { currentUser, bookings, vehicles, messages } = useApp();
      ...
      <div className="min-h-screen bg-background flex flex-col font-sans">
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <Card className="p-10 text-center max-w-sm rounded-3xl shadow-2xl border-none">
            ...
            <p className="text-muted-foreground mb-6">Log in as a renter to continue.</p>
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
      ...
      <div className="flex-1 pb-20 md:pb-12">
        <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {currentUser.kycStatus === 'verified'
                  ? <Badge className="bg-green-500 text-white gap-1 rounded-full text-[10px]"><ShieldCheck size={10} /> Verified</Badge>
                  : <Badge className="bg-amber-100 text-amber-700 border-amber-200 gap-1 rounded-full text-[10px]"><AlertCircle size={10} /> Pending KYC</Badge>
                }
              </div>
              <p className="text-muted-foreground">Hello, <span className="font-bold text-foreground">{currentUser.name}</span> 👋 Track your rentals.</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Active Rentals', value: activeBookings.length, icon: Activity, color: 'bg-green-500', light: 'bg-green-50 text-green-600' },
              { label: 'Pending', value: pendingBookings.length, icon: Clock, color: 'bg-amber-500', light: 'bg-amber-50 text-amber-600' },
              { label: 'Completed', value: completedBookings.length, icon: CheckCircle2, color: 'bg-blue-500', light: 'bg-blue-50 text-blue-600' },
              { label: 'Total Spend', value: `৳${totalSpend.toLocaleString()}`, icon: DollarSign, color: 'bg-primary', light: 'bg-primary/10 text-primary' },
            ].map((stat, i) => (
          ...
          <Tabs defaultValue="bookings" className="space-y-6">
            ...
            <TabsContent value="active" className="space-y-4 animate-in fade-in duration-300">
              {activeBookings.length > 0 ? activeBookings.map(booking => {
                const vehicle = vehicles.find(v => v.id === booking.vehicleId);
                return vehicle ? (
                  <Card key={booking.id} className="rounded-2xl border-none shadow-sm bg-white overflow-hidden">
                    <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-4 text-white flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-green-100">Active Rental</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-green-100">Until</p>
                        <p className="font-black text-sm">{new Date(booking.endDate).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="p-4 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{booking.pickupTime}</p>
                      </div>
                      <div className="flex gap-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-border px-6 py-2 flex justify-around items-center z-50 md:hidden">
        {[
          ...
          >
            <item.icon size={22} />
            {item.badge ? <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-black rounded-full flex items-center justify-center">{item.badge}</span> : null}
